    ...
    <div class="row center-page">
        <div class="col-xl-6 d-flex">
            <div class="card flex-fill">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.employee-update', $original->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row mb-3">
                            ...
                            <div class="col-lg-9">
                                <select name="branch_id" class="form-control select">
                                    ...
                                    @foreach ($branches as $item)
                                        <option value="{{ $item->id }}" {{ old('branch_id', $original->branch_id) == $item->id ? 'selected' : '' }}>
                                            {{ $item->branch_name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-lg-9">
                                <select name="route_id" class="form-control select">
                                    ...
                                    @foreach ($routes as $item)
                                        <option value="{{ $item->id }}" {{ old('route_id', $original->route_id) == $item->id ? 'selected' : '' }}>
                                            {{ $item->route_name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3">
                            <label class="col-lg-3 form-label">ID Proofs : </label>
                            <div class="col-lg-9">
                                <input type="file" class="form-control" name="id_proof[]" multiple>
                                <small class="text-muted">Recommended: JPG, PNG, GIF. Max: 2MB per file.</small>
                                @if($original->idProofs->count() > 0)
                                    <div class="mt-2">
                                        <h6>Current Proofs:</h6>
                                        <div class="d-flex flex-wrap gap-2">
                                            @foreach($original->idProofs as $proof)
                                                <div class="position-relative">
                                                    <a href="{{ asset('uploads/employees/' . $proof->image) }}" target="_blank">
                                                        <img src="{{ asset('uploads/employees/' . $proof->image) }}" width="50" height="50" class="img-thumbnail">
                                                    </a>
                                                </div>
                                            @endforeach
                                        </div>
